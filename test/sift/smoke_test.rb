...
class SmokeTest < Minitest::Test
  ...
  def test_sift_version_flag
    stdout, stderr, status = Timeout.timeout(10) { Open3.capture3("bundle", "exec", EXE_SIFT, "--version") }
  ...
  def test_sift_empty_queue_exits_gracefully
    Dir.mktmpdir("sift_smoke_") do |dir|
      ...
      _stdout, stderr, status = Timeout.timeout(10) { Open3.capture3(env, "bundle", "exec", EXE_SIFT) }
  ...
  def test_sq_list_empty_queue
    Dir.mktmpdir("sift_smoke_") do |dir|
      env = { "SIFT_QUEUE_PATH" => File.join(dir, "queue.jsonl") }
      stdout, stderr, status = Timeout.timeout(10) { Open3.capture3(env, "bundle", "exec", EXE_SQ, "list") }
